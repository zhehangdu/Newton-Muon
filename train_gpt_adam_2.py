...
class Hyperparameters:
    ...
    matrix_lr_mul = 1.0 # dedicated lr multiplier for the former Muon-managed block matrices
...
# split parameters so the former Muon-managed block matrices can use different Adam betas
hidden_matrix_params = [p for p in model.blocks.parameters() if p.ndim >= 2]
hidden_matrix_param_ids = {id(p) for p in hidden_matrix_params}
other_params = [p for p in model.parameters() if id(p) not in hidden_matrix_param_ids]

# assign a separate lr multiplier to the former Muon-managed block matrices
for param in hidden_matrix_params:
    param.lr_mul = args.matrix_lr_mul

# init the optimizer(s)
# Keep the original Adam-side beta settings for the original Adam parameters,
# but use default Adam betas for the former Muon-managed block matrices.
optimizer1 = DistAdam(other_params, lr=0.008, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0)
optimizer2 = DistAdam(hidden_matrix_params, lr=0.00055, betas=(0.9, 0.999), eps=1e-10, weight_decay=0.0)
